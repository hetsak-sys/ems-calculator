...
**Handoff note (updated):** the 2026-07-25 test-suite/refactor session is now fully closed — on-device verification against `docs/on_device_checklist.md` completed and confirmed 100% matching across all modules and both app-wide checks. This session's work is trustworthy end-to-end (automated tests + real-device UI). The next genuinely new work is scoping a domain per §5.1 (11kV/MV generation, or the next §5.2 candidate) — run the three-question checklist fresh, don't assume priority carries over from before this testing sweep.
