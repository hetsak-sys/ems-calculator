...
**Handoff note for next session:** §5.5 (Protection deepening — Earth Fault Protection expanded + Relay Selection) is now **confirmed**, not provisional — see §5.5 below. The next session should start there: run the actual feature-scoping conversation (what specific calculations, what UI, what standards detail) rather than re-running the §5.1 checklist, which is already settled.
...
Other than §5.5 (now confirmed, see below), none of the remaining candidates have been answered against this checklist yet — that's the next conversation, one domain at a time, not a batch decision.
...
| **Earth fault protection (expanded)** | NER sizing (exists), earth fault relay settings, sensitive earth fault (SEF) for HR systems | IEC 60255, MHSA | Extension of Protection — **confirmed next, see §5.5** |
| **Relay selection** | Overcurrent/earth fault/differential relay type by application, CT ratio/class matching | IEC 60255, IEC 61869 | Extension of Protection — **confirmed next, see §5.5** |
...
### 5.5 Deepening candidates within Protection — CONFIRMED (2026-07-23)

With Protection's six sub-tabs and the TCC Study now verified (§7), the two nearest "deepen before widening" candidates are **Earth Fault Protection (expanded)** and **Relay Selection**, both extensions of the existing Protection charter rather than new modules.

**Hertz confirmed this scoping on 2026-07-23 — this is settled, not provisional.** Against §5.1:
...
**Next session starts here.** The remaining work is a feature-level scoping conversation, not a repeat of the §5.1 checklist: what specific SEF/earth-fault calculations, what specific relay-selection reference tables, what the actual UI/sub-tab structure looks like, and which standards clauses ground each figure. Engine-first per HAIOS approach conventions — calculation engine with tests first, UI second, same pattern as `protectionCoordinationEngine.js`.
...
| **§5.5 Protection deepening scope (Earth Fault expanded + Relay Selection)** | **Confirmed (2026-07-23)** — Hertz confirmed the provisional priority/depth/module-vs-extension read as final. See §5.5. Ready for a feature-level scoping conversation in the next session | Not blocking — ready to start |
...
|---|---|---|---|
| No automated test suite | Solo-dev velocity; manual verification so far | Before any second developer joins, or before any calculation module is materially reworked | **Risky** — calculation logic is safety-relevant ([TST-2] priority 1 territory); this is the debt item closest to crossing into [DBT-4] forbidden-debt if left too long. Confirmed 2026-07-23: still just tracked, no active work wanted right now |
| No CI pipeline | No team-coordination problem yet | Before public distribution, or before a second developer joins | Cosmetic-to-risky. Confirmed 2026-07-23: still just tracked |
| No split `architecture.md`/`debt.md`/ADR index | This document currently absorbs those roles | Before the domain expansion (§5) begins in earnest — new architecture decisions (11 kV generation, reticulation) deserve real ADRs, not a table row. **Note: §5.5 is an extension of an existing module, not a new domain, so this trigger doesn't yet apply to the confirmed next work** | Risky, rising |
