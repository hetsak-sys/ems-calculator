...
**Handoff note for next session:** there is nothing queued from the Earth Fault / Relay Selection line of work — it's done end-to-end. Per §5.4 (explicit not-doing/sequencing), the next session should run a fresh scoping conversation before writing any code: either the 11kV/MV generation domain (§5.3) as its own conversation, or the next candidate from the §5.2 table, starting with the §5.1 three-question checklist rather than assuming priority.
